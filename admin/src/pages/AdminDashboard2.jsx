...
export default function AdminDashboard2() {
 ...
 const [pauseRequests, setPauseRequests] = useState([]);
  ...
  useEffect(() => {
    const anyModal = showPasskeyModal || showBroadcastModal || showCustomerDetail || showIndividualMessage || showRenew || showPaymentModal || showInvoiceModal || mobileNavOpen;
    document.body.style.overflow = anyModal ? 'hidden' : '';
    return () => { document.body.style.overflow = ''; };
  }, [showPasskeyModal, showBroadcastModal, showCustomerDetail, showIndividualMessage, showRenew, showPaymentModal, showInvoiceModal, mobileNavOpen]);
...
const derivePauseRequestsFromOrders = (orderList) => {
  ...
  const exportDeliveryLog = () => {
    const headers = ['Date', 'Membership ID', 'Customer', 'Plan', 'Slot', 'Customisation', 'Status', 'Notes'];
    const rows = deliveryLog.map(r => [
      deliveryDate, r.membershipId, r.name, r.plan, r.slot, r.customisation, r.status, r.notes
    ]);
    ...
    a.href = url;
    a.download = `Ryvive_Delivery_${deliveryDate}.csv`;
    a.click();
                            ...
                            <tr key={order._id} style={{ borderBottom: idx < filteredOrders.length - 1 ? `1px solid ${CARD_BORDER}` : 'none' }}>
                              ...
                              <td style={{ padding: '1.1rem 1rem' }}>
                                <div style={{ display: 'flex', gap: '0.5rem', flexWrap: 'wrap' }}>
                                  ...
                                  <button onClick={() => openInvoices(order)} style={{ ...ghostBtn, padding: '0.45rem 0.9rem', fontSize: '0.72rem' }}><FileText size={13} /> Invoice</button>
                                </div>
                              </td>
                            </tr>
                          );
                ...
                <div style={{ ...cardStyle, overflow: 'hidden' }}>
                  <div style={{ overflowX: 'auto' }}>
                    <table style={{ width: '100%', borderCollapse: 'collapse', minWidth: 860 }}>
                      <thead>
                        <tr style={{ background: CREAM_2 }}>
                          {['#', 'Customer', 'Membership ID', 'Plan', 'Slot', 'Customisation', 'Status', 'Notes'].map(h => (
                            <th key={h} style={{ padding: '0.95rem 1rem', textAlign: 'left', fontSize: '0.68rem', letterSpacing: '0.12em', textTransform: 'uppercase', fontWeight: 700, color: SAGE_DARK }}>{h}</th>
                          ))}
                        </tr>
                      </thead>
                      <tbody>
                        {deliveryLog.map((row, idx) => {
                          ...
                          return (
                            <tr key={row.orderId || idx} style={{ borderBottom: idx < deliveryLog.length - 1 ? `1px solid ${CARD_BORDER}` : 'none' }}>
                              <td style={{ padding: '0.85rem 1rem', fontSize: '0.82rem', color: 'rgba(42,37,32,0.5)', fontWeight: 600 }}>{idx + 1}</td>
                              <td style={{ padding: '0.85rem 1rem', fontWeight: 600, color: INK, fontSize: '0.9rem' }}>{row.name}</td>
                              <td style={{ padding: '0.85rem 1rem', fontSize: '0.82rem', color: 'rgba(42,37,32,0.6)' }}>{row.membershipId}</td>
                              ...
                              <td style={{ padding: '0.85rem 1rem', fontSize: '0.82rem', color: 'rgba(42,37,32,0.65)' }}>{row.slot}</td>
                              <td style={{ padding: '0.85rem 1rem', fontSize: '0.82rem', color: 'rgba(42,37,32,0.65)', maxWidth: 160 }}>
                                <input
                                  type="text"
                                  value={row.customisation}
                                  onChange={e => setDeliveryLog(prev => prev.map((r, i) => i === idx ? { ...r, customisation: e.target.value } : r))}
                                  style={{ border: 'none', outline: 'none', background: 'transparent', fontSize: '0.82rem', color: 'rgba(42,37,32,0.65)', width: '100%', fontFamily: 'Inter, sans-serif' }}
                                />
                              </td>
                              <td style={{ padding: '0.85rem 1rem' }}>
                                <select
                                  ...
                                  style={{ background: statusColor.bg, color: statusColor.fg, border: 'none', borderRadius: 3, padding: '0.3rem 0.65rem', fontSize: '0.75rem', fontWeight: 600, cursor: 'pointer', fontFamily: 'Inter, sans-serif' }}
                                >
                              ...
                              <td style={{ padding: '0.85rem 1rem' }}>
                                <input
                                  ...
                                  style={{ border: `1px solid ${CARD_BORDER}`, borderRadius: 3, padding: '0.35rem 0.65rem', fontSize: '0.82rem', color: INK, background: CREAM, fontFamily: 'Inter, sans-serif', minWidth: 120, outline: 'none' }}
                                />
                              </td>
                            </tr>
                          );
