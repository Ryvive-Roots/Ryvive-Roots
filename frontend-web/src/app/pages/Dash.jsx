...
import { User, Calendar, TrendingUp, Receipt, MessageCircle, Bell, LogOut, Edit3, Lock, Clock, CheckCircle, AlertCircle, Package, Pause, MapPin, RefreshCw, ChevronDown, Plus, X } from 'lucide-react';
...
const fetchNotifications = async () => {
  ...
  // Feedback state
  const [supportMode, setSupportMode] = useState("ticket"); // "ticket" | "feedback"
...
const [calendarDate, setCalendarDate] = useState(new Date());
  ...
  const [linkedMemberships, setLinkedMemberships] = useState([]);
  ...
  setLoading(false);
    ...
    // Load any linked membership IDs saved for this account
    try {
      const stored = JSON.parse(localStorage.getItem("linkedMemberships") || "[]");
      setLinkedMemberships(stored.filter((id) => id && id !== membershipId));
    } catch {
      setLinkedMemberships([]);
    }
  ...
  // ── Switch active membership/dashboard ──────────────────────────────────────
  const switchToMembership = (id) => {
    const current = localStorage.getItem("membershipId");
    if (!id || id === current) { setShowSwitcher(false); return; }

    // Save the membership being left so it stays in the switcher list
    let stored = [];
    try { stored = JSON.parse(localStorage.getItem("linkedMemberships") || "[]"); } catch {}
    const updated = Array.from(new Set([...stored, current, id])).filter((m) => m && m !== id);

    localStorage.setItem("membershipId", id);
    localStorage.setItem("linkedMemberships", JSON.stringify(updated));
    window.location.reload();
  };

  // ── Link a new membership ID to this account ────────────────────────────────
  const linkNewMembership = async () => {
    const id = newMembershipId.trim().toUpperCase();
    if (!id) { setLinkError("Please enter a membership ID."); return; }
    if (id === membershipId || linkedMemberships.includes(id)) {
      setLinkError("This membership is already linked.");
      return;
    }
    ...
    setLinkError("");
    try {
      const res = await fetch(`https://api.ryviveroots.com/api/user/orders?membershipId=${id}`);
      const data = await res.json();
      if (!data.success || !data.orders || data.orders.length === 0) {
        setLinkError("No subscription found for this membership ID.");
        setLinkLoading(false);
        return;
      }
      const stored = JSON.parse(localStorage.getItem("linkedMemberships") || "[]");
      const updated = Array.from(new Set([...stored, membershipId, id])).filter((m) => m && m !== membershipId);
      localStorage.setItem("linkedMemberships", JSON.stringify([...new Set([...stored, membershipId, id])]));
      setLinkedMemberships(updated);
      setNewMembershipId("");
      setLinkLoading(false);
    } catch (err) {
...
useEffect(() => {
  ...
  useEffect(() => {
    document.body.style.overflow = (showPauseModal || showRenewModal || showSummary || showSwitcher) ? "hidden" : "auto";
    return () => { document.body.style.overflow = "auto"; };
  }, [showPauseModal, showRenewModal, showSummary, showSwitcher]);
      ...
      <div style={S.layout}>
        ...
        <aside style={S.sidebar}>
          ...
          <div style={{ margin: "1.5rem 1rem", padding: "1.25rem", background: "linear-gradient(135deg,#f0f7ec 0%,#fef9f3 100%)", borderRadius: 10, border: "1px solid rgba(45,80,22,.1)" }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
              ...
              {linkedMemberships.length > 0 && (
                <button
                  ...
                  style={{ background: "white", border: "1px solid rgba(45,80,22,.15)", borderRadius: 7, padding: ".3rem", cursor: "pointer", display: "flex", alignItems: "center" }}
                >
                  <ChevronDown size={14} color="#3d6b1f" />
                </button>
              )}
            </div>
            ...
            <button
              onClick={() => setShowSwitcher(true)}
              style={{ marginTop: ".85rem", width: "100%", background: "transparent", border: "1px dashed rgba(45,80,22,.3)", borderRadius: 7, padding: ".4rem .6rem", cursor: "pointer", fontSize: ".75rem", fontWeight: 600, color: "#3d6b1f", fontFamily: "'Outfit',sans-serif", display: "flex", alignItems: "center", justifyContent: "center", gap: ".35rem" }}
            >
              <RefreshCw size={12} /> Switch / Link Membership
            </button>
          </div>
        </aside>
...
["Meals Remaining",  `${totalDays - daysCompleted} meals left`],
          ...
          {activeTab === "info" && (() => {
            ...
            const policyItems = [
              {
                icon: "👤", title: "Name Changes",
                rules: ["Cannot be edited directly by you", "Customer Support can update up to 2 times", "Further changes require Management approval"],
                tag: "Support Only"
              },
              {
                icon: "📧", title: "Email Address",
                rules: ["Allowed 2 changes total", "Second change requires 6-day gap from first", "After 2 changes, email is locked permanently"],
                tag: `${emailChanges}/2 used`
              },
              {
                icon: "📍", title: "Delivery Address",
                rules: ["Changes allowed Fri 11AM – Sat 5PM only", "New address takes effect from following Monday", "1-Month plan: 1 change | 3-Month plan: 3 changes"],
                tag: "Window: Fri–Sat"
              },
              {
                icon: "📱", title: "Mobile Number",
                rules: ["Allowed 2 changes total", "Second change requires 6-day gap from first", "Third change requires contacting Support"],
                tag: `${phoneChanges}/2 used`
              },
            ];
              ...
              <div>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: "1.5rem" }}>
                  ...
                  <div style={{ display: "flex", gap: ".75rem", alignItems: "center" }}>
                    <button
                      ...
                      style={{ background: "#f0f7ec", border: "1px solid rgba(45,80,22,.2)", borderRadius: 8, padding: ".5rem 1rem", cursor: "pointer", fontSize: ".82rem", fontWeight: 600, color: "#2d5016", fontFamily: "'Outfit',sans-serif", display: "flex", alignItems: "center", gap: ".4rem" }}
                    >
                      📋 Update Policy
                    </button>
                    {!editMode && canEdit && (
                      <button style={S.btnGold} onClick={() => setEditMode(true)}>
                        <Edit3 size={15} style={{ marginRight: ".4rem" }} /> Edit
                      </button>
                    )}
                  </div>
                </div>
                ...
                <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(200px,1fr))", gap: ".75rem", marginBottom: "1.5rem" }}>
                  {policyItems.map((p) => (
                    <div
                      ...
                      style={{ background: "white", border: "1px solid rgba(45,80,22,.1)", borderRadius: 10, padding: ".85rem 1rem", cursor: "pointer", transition: "all .2s" }}
                    >
                      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: ".35rem" }}>
                        <span style={{ fontSize: ".88rem", fontWeight: 600, color: "#2d5016" }}>{p.icon} {p.title}</span>
                        <span style={{ background: "#f0f7ec", color: "#3d6b1f", fontSize: ".7rem", fontWeight: 700, padding: ".15rem .5rem", borderRadius: 6 }}>{p.tag}</span>
                      </div>
                      <p style={{ margin: 0, fontSize: ".75rem", color: "#888" }}>{p.rules[0]}</p>
                    </div>
                  ))}
                </div>
                ...
                <div style={S.card}>
                  <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(270px,1fr))", gap: "1.25rem" }}>
                    ...
                    {/* Full Name — locked, support only */}
                    <div>
                      <label style={{ display: "flex", alignItems: "center", gap: ".4rem", fontSize: ".82rem", fontWeight: 600, color: "#666", marginBottom: ".4rem" }}>
                        Full Name <Lock size={13} color="#bbb" />
                        <span style={{ marginLeft: "auto", fontSize: ".7rem", color: "#999", background: "#f5f5f5", padding: ".1rem .45rem", borderRadius: 5 }}>Support Only</span>
                      </label>
                      <input type="text" value={`${user.firstName} ${user.lastName}`} disabled
                        style={{ width: "100%", padding: ".75rem", borderRadius: 8, border: "1px solid rgba(45,80,22,.15)", fontSize: ".92rem", fontWeight: 500, color: "#999", background: "#f5f5f5", cursor: "not-allowed", fontFamily: "'Outfit',sans-serif", outline: "none" }} />
                    </div>

                    {/* Membership ID */}
                    <div>
                      <label style={{ display: "flex", alignItems: "center", gap: ".4rem", fontSize: ".82rem", fontWeight: 600, color: "#666", marginBottom: ".4rem" }}>
                        Membership ID <Lock size={13} color="#bbb" />
                      </label>
                    ...
                    <div>
                      <label style={{ display: "flex", alignItems: "center", gap: ".4rem", fontSize: ".82rem", fontWeight: 600, color: "#666", marginBottom: ".4rem" }}>
                        Email Address
                        {emailLocked && <Lock size={13} color="#e53935" />}
                        <span style={{ marginLeft: "auto", fontSize: ".7rem", fontWeight: 700, color: emailLocked ? "#c62828" : emailChanges === 1 ? "#c8860f" : "#3d6b1f", background: emailLocked ? "#ffebee" : emailChanges === 1 ? "#fff8e1" : "#e8f5e9", padding: ".1rem .45rem", borderRadius: 5 }}>
                          {emailLocked ? "Locked" : `${emailChanges}/2 changes`}
                        </span>
                      </label>
                      <input type="email" value={formData.email} disabled={!canEditEmail}
                        onChange={(e) => setFormData({ ...formData, email: e.target.value })}
                        style={{ width: "100%", padding: ".75rem", borderRadius: 8, border: canEditEmail ? "2px solid #d4af37" : "1px solid rgba(45,80,22,.15)", fontSize: ".92rem", fontWeight: 500, color: emailLocked ? "#999" : "#2d5016", background: emailLocked ? "#f5f5f5" : "white", cursor: emailLocked ? "not-allowed" : canEditEmail ? "text" : "default", fontFamily: "'Outfit',sans-serif", outline: "none" }} />
                      {emailCooldown && (
                        <p style={{ margin: ".3rem 0 0 0", fontSize: ".75rem", color: "#c8860f" }}>⏳ Next change available in {6 - daysSinceEmailChange} day(s)</p>
                      )}
                      {emailLocked && (
                        <p style={{ margin: ".3rem 0 0 0", fontSize: ".75rem", color: "#c62828" }}>🔒 Email permanently locked after 2 changes</p>
                      )}
                    </div>
                    ...
                    <div>
                      <label style={{ display: "flex", alignItems: "center", gap: ".4rem", fontSize: ".82rem", fontWeight: 600, color: "#666", marginBottom: ".4rem" }}>
                        Phone Number
                        {phoneLocked && <Lock size={13} color="#e53935" />}
                        <span style={{ marginLeft: "auto", fontSize: ".7rem", fontWeight: 700, color: phoneLocked ? "#c62828" : phoneChanges === 1 ? "#c8860f" : "#3d6b1f", background: phoneLocked ? "#ffebee" : phoneChanges === 1 ? "#fff8e1" : "#e8f5e9", padding: ".1rem .45rem", borderRadius: 5 }}>
                          {phoneLocked ? "Contact Support" : `${phoneChanges}/2 changes`}
                        </span>
                      </label>
                      <input type="text" value={formData.phone} disabled={!canEditPhone}
                        onChange={(e) => setFormData({ ...formData, phone: e.target.value })}
                        style={{ width: "100%", padding: ".75rem", borderRadius: 8, border: canEditPhone ? "2px solid #d4af37" : "1px solid rgba(45,80,22,.15)", fontSize: ".92rem", fontWeight: 500, color: phoneLocked ? "#999" : "#2d5016", background: phoneLocked ? "#f5f5f5" : "white", cursor: phoneLocked ? "not-allowed" : canEditPhone ? "text" : "default", fontFamily: "'Outfit',sans-serif", outline: "none" }} />
                      {phoneCooldown && (
                        <p style={{ margin: ".3rem 0 0 0", fontSize: ".75rem", color: "#c8860f" }}>⏳ Next change available in {6 - daysSincePhoneChange} day(s)</p>
                      )}
                      {phoneLocked && (
                        <p style={{ margin: ".3rem 0 0 0", fontSize: ".75rem", color: "#c62828" }}>📞 Contact Support for further changes</p>
                      )}
                    </div>
                    ...
                    <div>
                      <label style={{ display: "flex", alignItems: "center", gap: ".4rem", fontSize: ".82rem", fontWeight: 600, color: "#666", marginBottom: ".4rem" }}>
                        Date of Birth <Lock size={13} color="#bbb" />
                      </label>
                    ...
                    <div>
                      <label style={{ display: "flex", alignItems: "center", gap: ".4rem", fontSize: ".82rem", fontWeight: 600, color: "#666", marginBottom: ".4rem" }}>
                        Delivery Address <Lock size={13} color="#bbb" />
                        <span style={{ marginLeft: "auto", fontSize: ".7rem", color: "#c8860f", background: "#fff8e1", fontWeight: 600, padding: ".1rem .45rem", borderRadius: 5 }}>Fri–Sat Window</span>
                      </label>
                      <input type="text" value={order.address ? `${order.address.house}, ${order.address.street}, ${order.address.city}` : "-"} disabled
                        style={{ width: "100%", padding: ".75rem", borderRadius: 8, border: "1px solid rgba(45,80,22,.15)", fontSize: ".92rem", fontWeight: 500, color: "#999", background: "#f5f5f5", cursor: "not-allowed", fontFamily: "'Outfit',sans-serif", outline: "none" }} />
                      <p style={{ margin: ".3rem 0 0 0", fontSize: ".75rem", color: "#888" }}>Changes via subscription settings · Fri 11AM–Sat 5PM only</p>
                    </div>

                    {/* Payment Method */}
                    <div>
                      <label style={{ display: "flex", alignItems: "center", gap: ".4rem", fontSize: ".82rem", fontWeight: 600, color: "#666", marginBottom: ".4rem" }}>
                        Payment Method <Lock size={13} color="#bbb" />
                      </label>
                ...
                {/* Policy Modal */}
                {showPolicyModal && (
                  <div style={S.overlay} onClick={() => { setShowPolicyModal(false); setActivePolicySection(null); }}>
                    <div style={{ ...S.modal, maxWidth: 520 }} onClick={(e) => e.stopPropagation()}>
                      <button style={{ position: "absolute", top: "1rem", right: "1rem", background: "none", border: "none", fontSize: "1.1rem", cursor: "pointer", color: "#999" }} onClick={() => { setShowPolicyModal(false); setActivePolicySection(null); }}>✕</button>
                      <h3 style={{ color: "#2d5016", fontSize: "1.2rem", fontWeight: 700, marginBottom: ".25rem" }}>Customer Information Update Policy</h3>
                      <p style={{ color: "#666", fontSize: ".83rem", marginBottom: "1.5rem" }}>We value transparency and security. Please review these rules before making changes.</p>
                      ...
                      {[
                        ...
                        return (
                          <div key={section.title} style={{ marginBottom: ".85rem", borderRadius: 10, border: isActive ? "1.5px solid #d4af37" : "1px solid rgba(45,80,22,.1)", overflow: "hidden" }}>
                            <div style={{ padding: ".85rem 1rem", background: isActive ? "#fffdf0" : "#f8fdf5", display: "flex", alignItems: "center", gap: ".5rem" }}>
                              <span style={{ fontSize: "1.1rem" }}>{section.icon}</span>
                              <span style={{ fontWeight: 700, color: "#2d5016", fontSize: ".95rem" }}>{idx + 1}. {section.title}</span>
                            </div>
                            <ul style={{ margin: 0, padding: "0 1rem 0 2rem" }}>
                              {section.rules.map((r, i) => (
                                <li key={i} style={{ padding: ".4rem 0", fontSize: ".85rem", color: "#555", borderBottom: i < section.rules.length - 1 ? "1px solid rgba(45,80,22,.06)" : "none" }}>{r}</li>
                              ))}
                            </ul>
                          </div>
                        );
                      ...
                      <div style={{ background: "#f0f7ec", borderRadius: 9, padding: ".85rem 1rem", marginTop: ".5rem", fontSize: ".8rem", color: "#555", lineHeight: 1.6 }}>
...
>
  <Receipt size={15} /> Download Receipt
          ...
          {activeTab === "support" && (() => {
            const feedbackTypes = [
              ...
              { id: "Feedback", icon: "💬", label: "Feedback", desc: "Share your experience or suggestions" },
            ...
            const handleFeedbackSubmit = async () => {
              ...
              try {
                const res = await fetch("https://api.ryviveroots.com/api/user/support", {
                  method: "POST",
                  headers: { "Content-Type": "application/json" },
                  body: JSON.stringify({ membershipId, type: feedbackType, message: feedbackText, rating: feedbackRating }),
                });
                const data = await res.json();
            ...
            return (
              <div>
                ...
                <p style={{ margin: "0 0 1.75rem 0", color: "#666" }}>Get help or share your experience</p>

                {/* Contact strip */}
                <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(220px,1fr))", gap: ".85rem", marginBottom: "1.75rem" }}>
                  {[
                    { icon: "📧", label: "Email Support", val: "customersupport@ryviveroots.com", sub: "Replies within 24 hours" },
                    { icon: "📞", label: "Phone Support", val: "+91 97656 00701", sub: "Mon–Sat, 9AM–6PM" },
                    { icon: "💬", label: "WhatsApp", val: "Chat with us", sub: "Quick responses" },
                  ].map((c) => (
                    <div key={c.label} style={{ background: "white", border: "1px solid rgba(45,80,22,.1)", borderRadius: 12, padding: "1rem 1.1rem", display: "flex", alignItems: "flex-start", gap: ".75rem" }}>
                      <div style={{ background: "#e8f5e9", borderRadius: 9, padding: ".6rem", fontSize: "1.1rem", flexShrink: 0 }}>{c.icon}</div>
                      <div>
                        <p style={{ margin: "0 0 .15rem 0", fontSize: ".78rem", color: "#888", fontWeight: 500 }}>{c.label}</p>
                        <p style={{ margin: "0 0 .1rem 0", fontSize: ".9rem", fontWeight: 700, color: "#2d5016" }}>{c.val}</p>
                        <p style={{ margin: 0, fontSize: ".75rem", color: "#aaa" }}>{c.sub}</p>
                      </div>
                    </div>
                  ))}
                </div>
                ...
                <div style={S.card}>
                  ...
                  {/* Pill toggle */}
                  <div style={{ display: "flex", background: "#f0f7ec", borderRadius: 10, padding: "4px", marginBottom: "1.4rem", width: "fit-content" }}>
                    {[{ id: "ticket", label: "Raise a Ticket" }, { id: "feedback", label: "Share Feedback" }].map((tab) => (
                      <button
                        ...
                        style={{
                          padding: ".45rem 1.1rem", borderRadius: 7, border: "none", cursor: "pointer",
                          fontFamily: "'Outfit',sans-serif", fontSize: ".88rem", fontWeight: supportMode === tab.id ? 700 : 500,
                          background: supportMode === tab.id ? "white" : "transparent",
                          color: supportMode === tab.id ? "#2d5016" : "#888",
                          boxShadow: supportMode === tab.id ? "0 1px 4px rgba(0,0,0,.08)" : "none",
                          transition: "all .15s",
                        }}
                      >
                        {tab.label}
                  ...
                  {feedbackSubmitted ? (
                    <div style={{ textAlign: "center", padding: "2rem 1rem" }}>
                      ...
                      <p style={{ fontWeight: 700, color: "#2d5016", fontSize: "1.1rem", margin: "0 0 .35rem 0" }}>
                        {supportMode === "feedback" ? "Feedback Received" : "Ticket Submitted"}
                      </p>
                      <p style={{ color: "#666", fontSize: ".88rem", margin: "0 0 1.25rem 0" }}>
                        {supportMode === "feedback" ? "Thank you — your feedback helps us improve." : "We'll get back to you within 24 hours."}
                      </p>
                      <button style={S.btnGold} onClick={() => setFeedbackSubmitted(false)}>Submit Another</button>
                    </div>
                  ) : supportMode === "ticket" ? (
                    <>
                      <p style={{ color: "#888", fontSize: ".83rem", marginBottom: "1.25rem" }}>We read every message and respond within 24 hours.</p>
                      {/* Type selector */}
                      <div style={{ display: "grid", gridTemplateColumns: "repeat(2,1fr)", gap: ".65rem", marginBottom: "1.25rem" }}>
                        {[
                          ...
                          <div
                            ...
                            style={{ border: `${feedbackType === t.id ? "2" : "1"}px solid ${feedbackType === t.id ? "#d4af37" : "rgba(45,80,22,.15)"}`, borderRadius: 10, padding: ".85rem .75rem", cursor: "pointer", background: feedbackType === t.id ? "#fffdf0" : "white", transition: "all .15s" }}
                          >
                            <div style={{ fontSize: "1.3rem", marginBottom: ".3rem" }}>{t.icon}</div>
                            <p style={{ margin: "0 0 .15rem 0", fontWeight: 700, fontSize: ".88rem", color: feedbackType === t.id ? "#2d5016" : "#444" }}>{t.label}</p>
                            <p style={{ margin: 0, fontSize: ".72rem", color: "#999", lineHeight: 1.3 }}>{t.desc}</p>
                          </div>
                        ))}
                      </div>
                      <label style={{ fontSize: ".82rem", fontWeight: 600, color: "#666", display: "block", marginBottom: ".4rem" }}>
                        {feedbackType === "Complaint" ? "Describe the issue" : "Your question"}
                      </label>
                      <textarea
                        value={feedbackText}
                        ...
                        style={{ width: "100%", minHeight: 110, padding: ".85rem", border: "1px solid rgba(45,80,22,.2)", borderRadius: 9, fontSize: ".9rem", fontFamily: "'Outfit',sans-serif", resize: "vertical", marginBottom: "1rem", color: "#2d5016", outline: "none", lineHeight: 1.6 }}
                      />
                      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                        <p style={{ margin: 0, fontSize: ".78rem", color: "#aaa" }}>{feedbackText.length} characters</p>
                        <button style={S.btnGreen} onClick={handleFeedbackSubmit}>Submit Ticket →</button>
                      </div>
                    </>
                  ) : (
                    <>
                      <p style={{ color: "#888", fontSize: ".83rem", marginBottom: "1.25rem" }}>Your experience helps us get better. Every word counts.</p>
                      {/* Star rating */}
                      <div style={{ marginBottom: "1.1rem" }}>
                        <label style={{ fontSize: ".82rem", fontWeight: 600, color: "#666", display: "block", marginBottom: ".5rem" }}>Rate your experience</label>
                        <div style={{ display: "flex", gap: ".4rem" }}>
                          {[1, 2, 3, 4, 5].map((star) => (
                            <span
                              key={star}
                              onClick={() => setFeedbackRating(star)}
                              style={{ fontSize: "2rem", cursor: "pointer", color: star <= feedbackRating ? "#d4af37" : "#ddd", transition: "color .15s" }}
                            >★</span>
                          ))}
                        </div>
                      </div>
                      <label style={{ fontSize: ".82rem", fontWeight: 600, color: "#666", display: "block", marginBottom: ".4rem" }}>Your feedback</label>
                      <textarea
                        value={feedbackText}
                        onChange={(e) => setFeedbackText(e.target.value)}
                      ...
                      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                        <p style={{ margin: 0, fontSize: ".78rem", color: "#aaa" }}>{feedbackText.length} characters</p>
                        <button style={S.btnGold} onClick={handleFeedbackSubmit}>Submit Feedback →</button>
                      </div>
                    </>
                  )}
                </div>
                ...
                {/* Tickets list */}
                <h3 style={{ color: "#2d5016", fontSize: "1.1rem", fontWeight: 600, margin: "1.75rem 0 .85rem 0" }}>Your Tickets</h3>
                <div style={{ ...S.card, padding: 0, overflow: "hidden" }}>
  ...
  .map((plan) => {
      ...
      {/* ── Membership Switcher Modal ── */}
      {showSwitcher && (
        <div style={S.overlay} onClick={() => setShowSwitcher(false)}>
          <div style={{ ...S.modal, maxWidth: 440 }} onClick={(e) => e.stopPropagation()}>
            <button style={{ position: "absolute", top: "1rem", right: "1rem", background: "none", border: "none", fontSize: "1.1rem", cursor: "pointer", color: "#999" }} onClick={() => setShowSwitcher(false)}>✕</button>
            <h3 style={{ color: "#2d5016", fontSize: "1.2rem", fontWeight: 700, marginBottom: ".25rem" }}>Switch Membership</h3>
            <p style={{ color: "#666", fontSize: ".85rem", marginBottom: "1.25rem" }}>
              Have more than one subscription? Switch between dashboards or link a new membership ID below.
            </p>

            {/* Current membership */}
            <div style={{ background: "#f0f7ec", border: "1.5px solid #d4af37", borderRadius: 10, padding: ".85rem 1rem", marginBottom: ".75rem", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
              <div>
                <p style={{ margin: "0 0 .15rem 0", fontSize: ".7rem", color: "#888", fontWeight: 600 }}>CURRENTLY VIEWING</p>
                <p style={{ margin: 0, fontSize: ".95rem", fontWeight: 700, color: "#2d5016" }}>{membershipId}</p>
                <p style={{ margin: ".15rem 0 0 0", fontSize: ".78rem", color: "#3d6b1f", fontWeight: 600 }}>RYVIVE {basePlan} · {durationMonths}M</p>
              </div>
              <span style={{ background: "#d4af37", color: "#2d5016", fontSize: ".68rem", fontWeight: 700, padding: ".2rem .55rem", borderRadius: 6 }}>Active</span>
            </div>

            {/* Other linked memberships */}
            {linkedMemberships.length > 0 && (
              <div style={{ marginBottom: "1.25rem" }}>
                {linkedMemberships.map((id) => (
                  <div
                    key={id}
                    onClick={() => switchToMembership(id)}
                    style={{ background: "white", border: "1px solid rgba(45,80,22,.15)", borderRadius: 10, padding: ".85rem 1rem", marginBottom: ".5rem", display: "flex", justifyContent: "space-between", alignItems: "center", cursor: "pointer", transition: "all .15s" }}
                  >
                    <div>
                      <p style={{ margin: 0, fontSize: ".95rem", fontWeight: 700, color: "#2d5016" }}>{id}</p>
                      <p style={{ margin: ".1rem 0 0 0", fontSize: ".75rem", color: "#999" }}>Tap to switch to this dashboard</p>
                    </div>
                    <RefreshCw size={16} color="#3d6b1f" />
                  </div>
                ))}
              </div>
            )}
            ...
            {/* Add new membership */}
            <div style={{ borderTop: "1px solid rgba(45,80,22,.08)", paddingTop: "1.1rem" }}>
              <label style={{ fontSize: ".82rem", fontWeight: 600, color: "#666", display: "block", marginBottom: ".4rem" }}>
                Link Another Membership
              </label>
              <div style={{ display: "flex", gap: ".5rem" }}>
                <input
                  type="text"
                  value={newMembershipId}
                  onChange={(e) => { setNewMembershipId(e.target.value); setLinkError(""); }}
                  placeholder="e.g. RR-2024-0482"
                  style={{ flex: 1, padding: ".7rem .85rem", borderRadius: 8, border: "1px solid rgba(45,80,22,.2)", fontSize: ".88rem", fontFamily: "'Outfit',sans-serif", color: "#2d5016", outline: "none" }}
                />
                <button
                  onClick={linkNewMembership}
                  disabled={linkLoading}
                  style={{ ...S.btnGreen, padding: ".7rem 1rem", display: "flex", alignItems: "center", gap: ".3rem", opacity: linkLoading ? .6 : 1, cursor: linkLoading ? "not-allowed" : "pointer" }}
                >
                  <Plus size={15} /> {linkLoading ? "Checking..." : "Link"}
                </button>
              </div>
              {linkError && (
                <p style={{ margin: ".5rem 0 0 0", fontSize: ".78rem", color: "#c62828" }}>{linkError}</p>
              )}
              <p style={{ margin: ".6rem 0 0 0", fontSize: ".75rem", color: "#aaa" }}>
                Enter the Membership ID of any other active subscription you hold — once linked, you can switch between dashboards anytime.
              </p>
            </div>
          </div>
        </div>
